...
"""
get image  424*240
load detect    resize 128*128
object detect   resize  256*256
"""
'''##########################################################load  libery##########################################################'''
import os
import v4l2capture
...
'''##########################################################load detect##########################################################'''
'''###1.get image from picture 2.resize to 128*128###'''
def dataset(video): 
...
def THREAD(run):
    ...
    while run.value == True:
        print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxget data",datetime.now()-nowtime2)
        nowtime2 = datetime.now()
        img = dataset(video)
...
if __name__ == "__main__": 
    ...
    lib = so(lib_path)
    '''##########################################################load  detect##########################################################'''
    predictor = load_model();##########################define  paddle*
    '''##########################################################object  detect##########################################################'''
    ...
    TH_1.start()'''
    process_car = multiprocessing.Process(target=THREAD,args=(RUN))
    process_car.start()
    time.sleep(1)
    try:
    #while 1:
        ...
        yuzhi_4 = 0
        while RUN.value == True:
            print("????????????????????????????????????????????????????????????????????????????????main cessing",datetime.now()-nowtime2)
            nowtime2 = datetime.now()
     
            '''tensor_img= read_image()  #  resize image                     
            tensor = pm.PaddleTensor()
            tensor.dtype =pm.PaddleDType.FLOAT32
