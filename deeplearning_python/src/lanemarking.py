...
Speed[0]  = Speed[1]+int(sys.argv[1])*4

'''
camera.value = sys.argv[1]
output_data.value = sys.argv[3]

Speed[1]  = 1500
Speed[0]  = Speed[1]+int(sys.argv[0])*4
serial.value = sys.argv[2]
'''
'''创建一个互斥锁，默认是没有上锁的'''
...
def getvalue():
    ...
    fcntl.ioctl(jsdev, 0x80006a13 + (0x10000 * len(buf)), buf)
    ...
    buf = array.array('B', [0])
    fcntl.ioctl(jsdev, 0x80016a11, buf)  # JSIOCGAXES
    num_axes = buf[0]

    buf = array.array('B', [0])
    fcntl.ioctl(jsdev, 0x80016a12, buf) # JSIOCGBUTTONS
    num_buttons = buf[0]

    # Get the axis map
    buf = array.array('B', [0] * 0x40)
    fcntl.ioctl(jsdev, 0x80406a32, buf) #JSIOCGAXMAP
    ...
    fcntl.ioctl(jsdev, 0x80406a34, buf) # JSIOCGBTNMAP
    ...
    try:
        ...
        while(1):
            if(Status.value == 0):
                ...
                #path_zip = path
                path_data = path + "/data"
                #zip_name =  firstbatch.zip
                zip_file_path(path_data, path_zip, "firstbatch.zip")
                print("over zip")
                
                path_file_number=glob.glob(path_data + "/img"+'/*.jpg')
                print('path_file_number:',len(path_file_number))
                ...
                r = requests.put('http://192.168.1.254/api/system/status',
                                 headers={'Content-Type': 'application/json'},
                                 data=json.dumps({'status': 4, 'data': {'pictures': len(path_file_number)}}))

                print(r.status_code)
                print(r.json())
                '''
                while(1):
                    print("guess  which error")
                '''
                break
    except:
        RUN.value = False
