...
predictor1 = CreatePaddlePredictor(config);
#return predictor;
#predictor1 = load_model(path)
...
def read_image(img_path):
    ...
    if img.mode != 'RGB':
        img = img.convert('RGB')
    img = np.array(img).astype('float32').transpose(( 0, 1 , 2))  # HWC to CHW
    img -= 127.5
    img *= 0.007843
    img = img[np.newaxis, :]
...
def infer(image_path):
    ...
    '''
    ##############################paddlelite
    im = predictor1.get_input(0);
    im.resize((1, 3, 224, 224));
    z = np.zeros((1,224, 224,3))
    print("size::::::::",tensor_img.shape)
    print("z___size::::::::",z.shape)
    z[0, 0:tensor_img.shape[1], 0:tensor_img.shape[2] + 0, 0:tensor_img.shape[3]] = tensor_img
    z = z.reshape(1, 3, 224, 224);
    im.set_data(z)
    predictor1.run();
    batch_outputs = predictor1.get_output(0);
